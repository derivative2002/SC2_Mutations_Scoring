...
class Trainer:
    ...
    def __init__(self,
        ...
        # 记录模型图
        dummy_input = {
            'map_ids': torch.zeros(1, dtype=torch.long, device=device),
            'commander_ids': torch.zeros((1, 2), dtype=torch.long, device=device),
            'mutation_ids': torch.zeros((1, 8), dtype=torch.long, device=device),
            'mutation_mask': torch.ones((1, 8), dtype=torch.float, device=device),
            'ai_ids': torch.zeros(1, dtype=torch.long, device=device)
        }
